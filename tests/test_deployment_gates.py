...
def test_production_deploy_requires_explicit_approval(tmp_path: Path) -> None:
    ...
    approved_one = engine.approve_production(run_id=state.run_id, approved_by="oncall", ticket_id="CHG-1", reason="CAB")
    ...
    approved_two = engine.approve_production(run_id=state.run_id, approved_by="sre", ticket_id="CHG-2", reason="Second eye")
...
def test_production_approval_expiry_blocks_deploy(tmp_path: Path) -> None:
    ...
    state = asyncio.run(engine.run(flow=FlowType.PLAN, request="Plan only", allow_production_deploy=True))
    engine.approve_production(run_id=state.run_id, approved_by="oncall", ticket_id="CHG-1", reason="CAB", expires_in_minutes=-1)
    engine.approve_production(run_id=state.run_id, approved_by="sre", ticket_id="CHG-2", reason="Second eye", expires_in_minutes=60)
